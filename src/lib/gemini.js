...
async function callOpenRouter(model, apiKey) {
...
export async function generateActivitiesFromAI() {
    ...
    for (const model of FREE_MODELS) {
        try {
            ...
            const validCategories = ['physical', 'social', 'hydration', 'mindfulness'];
            const activities = parsed
                .filter((item) => typeof item === 'object' && item !== null)
                .map((item, index) => {
                let catStr = String(item.category || '').toLowerCase();
                if (catStr === 'stretch')
                    catStr = 'physical'; // Map legacy prompt
                const category = validCategories.includes(catStr)
                    ? catStr
                    : 'physical';
                return {
